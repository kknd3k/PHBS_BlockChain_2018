...
<br> There is only a small number of literatures about bitcoin price formation. It is hard to apply the classic economic model on this topic if you want to get some new results. Therefore, researchers are trying some model on science, like signal analysis. But one weakness is based on different model, you will get opposite conlusion on one factor. Since we don't have a model with predict power, we can't test which conclusion is right.
